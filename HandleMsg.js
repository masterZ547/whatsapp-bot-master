...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'Error: Sorry, this command can only be used within groups!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Error: Failed, this command can only be used by group admins!', id)
            await aruga.sendText(from, menuId.textAdmin())
            break
        case 'donate':
        case 'tahunbaru':
            await aruga.sendText(from, menuId.texttahunbaru())
            break
        case 'ownerbot':
            await aruga.sendContact(from, ownerNumber)
            .then(() => aruga.sendText(from, 'System: If you want to request a feature, please chat with the owner number!'))
            break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `System: If you want to invite the bot to the group please invite or with\nType ${prefix}join [link group]`, id)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await aruga.inviteInfo(linkgrup)
            if (!islink) return aruga.reply(from, 'System: Sorry the group link is wrong! please send us the correct link', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, 'System: Successfully joined the group via link!')
                          await aruga.sendText(chekgrup.id, `System: Hai Friend~, Im Gabut BOT. To find out the commands on this bot type ${prefix}menu`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `System: Sorry, the group on this bot is full\nMax Group is: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `System: Sorry, BOT wil not join if the group members do not exceed ${memberLimit} people`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await aruga.reply(from, 'System: Successfully joined the group via link!', id)
                      })
                      .catch(() => {
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                if (isMedia || isQuotedImage) {
                    ...
                    console.log(err)
	   	            await aruga.reply(from, 'Error: Error: Sorry, todays usage limit has reached the maximum', id)
                    }
                }
            } else if (args.length === 1) {
                if (!isUrl(url)) { await aruga.reply(from, 'Error: Sorry, the link you submitted is invalid.', id) }
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? aruga.sendText(from, 'Error: Sorry, the link you submitted does not contain an image.')
                    : aruga.reply(from, 'Here\'s your sticker')).then(() => console.log(`Sticker Processed for ${processTime(t, moment())} Second`))
            } else {
                await aruga.reply(from, `Tidak ada gambar! To use ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
            }
            break
        case 'stickergif':
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, 'Error: Sorry the file is too big!', id)
                        })
                    })
                  } else {
                    aruga.reply(from, `[❗] System: Send a gif with a caption. *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
		    aruga.reply(from, `[❗] System: Send a gif with a caption. *${prefix}stickergif*`, id)
	        }
            break
        case 'stikergiphy':
        case 'stickergiphy':
            if (args.length !== 1) return aruga.reply(from, `Error: Sorry, the message format is wrong.\nKetik pesan dengan ${prefix}stickergiphy <link_giphy>`, id)
            const isGiphy = url.match(new RegExp(/https?:\/\/(www\.)?giphy.com/, 'gi'))
            const isMediaGiphy = url.match(new RegExp(/https?:\/\/media.giphy.com\/media/, 'gi'))
            if (isGiphy) {
                const getGiphyCode = url.match(new RegExp(/(\/|\-)(?:.(?!(\/|\-)))+$/, 'gi'))
                if (!getGiphyCode) { return aruga.reply(from, 'Error: Failed to retrieve the giphy code', id) }
                ...
                if (!gifUrl) { return aruga.reply(from, 'Error: Failed to retrieve the giphy code', id) }
                ...
                .catch(() => {
                    aruga.reply(from, `Error: Something went wrong!`, id)
                })
            } else {
                await aruga.reply(from, 'Error: Sorry, the giphy sticker command can only use the link from giphy.  [Giphy Only]', id)
            }
            break
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    ...
                    .catch(() => {
                        aruga.reply(from, 'Error: Something went wrong!')
                    })
            } else {
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                aruga.reply(from, 'Process Loading...', id)
                try {
                    const hasilqmaker = await images.quote(quotes, author, theme)
                    aruga.sendFileFromUrl(from, `${hasilqmaker}`, '', 'System: Hello, Thank you for using Gabut Bot.', id)
                } catch {
                    aruga.reply('System: Failed to complete the process, please try again.', id)
                }
            } else {
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'System: Hello, Thank you for using Gabut Bot.', id)
            .catch(() => {
                aruga.reply(from, 'Error: Something went wrong!', id)
            })
            break
        ...
        //Media
        case 'ytmp3':
            if (args.length == 0) return aruga.reply(from, `System: To download songs from youtube\nketik: ${prefix}ytmp3 [link_yt]`, id)
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				if (res.status == 'error') return aruga.sendFileFromUrl(from, link, '', 'Error: Sorry, your url could not be found', id)
				await aruga.sendFileFromUrl(from, linkhd, '', 'here your video', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', 'here your video', id)
					.catch(() => {
						aruga.reply(from, 'Error: Sorry, your url could not be found', id)
					})
				})
			})
			break
		...
		//Primbon Menu
		case 'artinama':
			if (args.length == 0) return aruga.reply(from, `To find out the meaning of someones name\nketik ${prefix}artinama namakamu`, id)
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                aruga.reply(from, `Sorry the query is not available. Please type ${prefix}anime untuk melihat list query`)
            }
            break
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                aruga.reply(from, `Sorry the query is not available. Please type ${prefix}kpop untuk melihat list query`)
            }
            break
        case 'memes':
        ...
        case 'nekopoi':
            ...
            break
        case 'stalkig':
            if (args.length == 0) return aruga.reply(from, `System: To stalk someone's Instagram account\nketik ${prefix}stalkig [username]\nExample: ${prefix}stalkig ini.arga`, id)
            ...
            break
        case 'wiki':
            if (args.length == 0) return aruga.reply(from, `System: To search for a word from wikipedia\nketik: ${prefix}wiki [kata]`, id)
            ...
            break
        case 'cuaca':
            if (args.length == 0) return aruga.reply(from, `System: To see the weather in an area\nketik: ${prefix}cuaca [daerah]`, id)
        ...
        case 'lirik':
            ...
            break
        case 'chord':
            if (args.length == 0) return aruga.reply(from, `System: To search for the lyrics and chords of a song\bketik: ${prefix}chord [judul_lagu]`, id)
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		aruga.reply(from, 'Error: Sorry, I dont know what anime this is, make sure the image to be searched is not blurry/cut', id)
                	}
        ...
        // Other Command
        case 'resi':
            if (args.length !== 2) return aruga.reply(from, `Error: Sorry, the message format is wrong.\nSilahkan ketik pesan dengan ${prefix}resi <kurir> <no_resi>\n\nKurir yang tersedia:\njne, pos, tiki, wahana, jnt, rpx, sap, sicepat, pcp, jet, dse, first, ninja, lion, idl, rex`, id)
        ...
        case 'translate':
            ...
            if (!quotedMsg) return aruga.reply(from, `Error: Sorry, the message format is wrong.\nSilahkan reply sebuah pesan dengan caption ${prefix}translate <kode_bahasa>\ncontoh ${prefix}translate id`, id)
		...
		case 'covidindo':
			...
			break
        case 'ceklokasi':
            if (quotedMsg.type !== 'location') return aruga.reply(from, `Error: Sorry, the message format is wrong.\nKirimkan lokasi dan reply dengan caption ${prefix}ceklokasi`, id)
        ...
        case 'shortlink':
            ...
            if (!isUrl(args[0])) return aruga.reply(from, 'Error: Sorry, the url you submitted is invalid.', id)
		...
		case 'klasmen':
            ...
            let textKlas = "*Temporary Fines Standings*\n"
            let i = 1;
            urut.forEach((klsmn) => {
	    ...
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Error: Sorry, this command can only be used within groups!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Error: Failed, this command can only be used by group admins!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Error: Failed, please add the bot as a group admin!', id)
	        if (args.length !== 1) return aruga.reply(from, `To use ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
                    await aruga.addParticipant(from,`${args[0]}@c.us`)
                } catch {
                    aruga.reply(from, 'Error: Cannot add target', id)
                }
            break
        case 'kick':
            if (!isGroupMsg) return aruga.reply(from, 'Error: Sorry, this command can only be used within groups!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Error: Failed, this command can only be used by group admins!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Error: Failed, please add the bot as a group admin!', id)
            if (mentionedJidList.length === 0) return aruga.reply(from, 'Error: Sorry, the message format is wrong.\nSilahkan tag satu atau lebih orang yang akan dikeluarkan', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Error: Sorry, the message format is wrong.\nTidak dapat mengeluarkan akun bot sendiri', id)
        ...
        case 'demote':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Error: Failed, this command can only be used by group admins!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Error: Failed, please add the bot as a group admin!', id)
            if (mentionedJidList.length !== 1) return aruga.reply(from, 'Error: Sorry, only 1 user can be demonstrated', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await aruga.reply(from, 'Error: Sorry, the user is not an admin yet.', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Error: Sorry, the message format is wrong.\nTidak dapat mendemote akun bot sendiri', id)
            await aruga.demoteParticipant(groupId, mentionedJidList[0])
            await aruga.sendTextWithMentions(from, `System: Request accepted, delete position @${mentionedJidList[0].replace('@c.us', '')}.`)
            break
        case 'bye':
            if (!isGroupMsg) return aruga.reply(from, 'Error: Sorry, this command can only be used within groups!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Error: Failed, this command can only be used by group admins!', id)
		...
		case 'setprofile':
			...
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				? aruga.reply(from, 'Error: Sorry, the link you submitted does not contain an image.', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *Pemberitahuan* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *Pemberitahuan* 〙\n\n${msg}`)
            }
            aruga.reply(from, 'System: Broadcast Success!', id)
            break
