...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('GABUT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('Gillan', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai Friend~, Im GABUT BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            ...
            await aruga.sendTextWithMentions(event.chat, `Hello, Welcome to the group gabut @${event.who.replace('@c.us', '')} \n\nHave fun with us✨`)
        }
    ...
    aruga.onIncomingCall(async (callData) => {
        ...
        await aruga.sendText(callData.peerJid, 'Error: Sorry you cant receive calls.\n\n-bot')
        .then(async () => {
    ...
    aruga.onMessage(async (message) => {
        aruga.getAmountOfLoadedMessages() // menghapus pesan cache jika sudah 3000 pesan.
            .then((msg) => {
                if (msg >= 3000) {
                    console.log('[GABUT]', color(`Loaded Message Reach ${msg}, cuting message cache...`, 'yellow'))
                    aruga.cutMsgCache()
                }
            })
